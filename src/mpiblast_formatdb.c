...
/*****************************************************************************
   ...
   ***************************************************************************
   ...
   $Log: mpiblast_formatdb.c,v $
   Revision 1.1.1.1  2008/01/16 23:10:07  hlin2


   Revision 1.2.8.3  2008/01/15 22:52:27  hlin2
   *** empty log message ***

   Revision 1.2.8.2  2007/08/09 15:59:15  hlin2
   hide arguments

   Revision 1.2.4.1  2007/06/30 16:20:22  hlin2
   *** empty log message ***

   Revision 1.2.2.1  2007/03/22 18:54:40  hlin2
   block recv results

   Revision 1.1.1.1.4.2  2007/01/21 20:24:36  hlin2
   upgraded ncbi toolkit may2006

   Revision 1.4  2005/12/20 02:51:54  koadman
   bandaid to build against new toolbox

   Revision 1.3  2004/12/03 20:25:31  koadman
   oops!  forgot to change fdbp to fdbp_array when updating for 2.2.10

   Revision 1.2  2004/12/03 02:26:52  koadman
   Updated with changes for blast 2.2.10

   Revision 1.1  2004/08/29 18:47:31  koadman
   Version of formatdb that guarantees a particular # of frags.  Up-to-date with blast 2.2.9 release.
   ...
   Add oidlist and gifile to FD_CreateAliasFileEx2
...
/* mpiBLAST specific <<<<< */
    {"Number of database volumes",
     "0", "1","250",TRUE,'N',ARG_INT, 0.0,0,NULL},
/* mpiBLAST specific >>>>> */
#if 0
...
/* mpiBLAST specific <<<<< */
/**
 * mpiBLAST support function
 * Clone an existing formatdb options structure, creating new buffers for strings
 */
FDB_optionsPtr FDBOptionsClone( FDB_optionsPtr old_opts )
{
    ...
    if ((options = (FDB_optionsPtr)MemNew(sizeof(FDB_options))) == NULL) {
        ErrPostEx(SEV_ERROR, 0, 0, "FDBOptionsClone: Out of memory");
        return NULL;
    }
	MemCpy( options, old_opts, sizeof(FDB_options) );

	/* allocate new storage space for strings */
	if( old_opts->db_file )
	    options->db_file = StringSave(old_opts->db_file);
	if( old_opts->db_title )
	    options->db_title = StringSave(old_opts->db_title);
	if( old_opts->base_name )
	    options->base_name = StringSave(old_opts->base_name);
	if( old_opts->alias_file_name )
	    options->alias_file_name = StringSave(old_opts->alias_file_name);
...
/**
 * mpiBLAST support function
 * Creates a specified number of blast database volumes
 */
Int4 FDBCreateMultipleVolumes(FormatDBPtr* fdbp_array, 
				FDB_optionsPtr options,
			       Int4 volume_count)
{
	FDB_optionsPtr tmp_opts;
	Int4 volumeI;
	Char db_base[PATH_MAX];
	Char dbnamebuf[PATH_MAX];

	strncpy( db_base, options->base_name, PATH_MAX );
	
	for( volumeI = 0; volumeI < volume_count; volumeI++ ){
		
		/* set the volume ID and name */
		sprintf(dbnamebuf, "%s.%03ld", db_base, (long) options->volume);
		MemFree(options->base_name);
		options->base_name = StringSave(dbnamebuf);
		
		/* create an options data structure for this volume */
		tmp_opts = FDBOptionsClone( options );
		
		/* init the data structure */
		fdbp_array[ volumeI ] = FormatDBInit(tmp_opts);
		if( fdbp_array[ volumeI ] == NULL ){
			return 9;
		}
	...
	/* in the NCBI code, options->volume contains one less than the
	 * total number of volumes, subtract 1 for consistency */
	options->volume--;
...
/**
 * mpiBLAST support function
 * Closes a set of blast database volumes
 */
Int4 FDBCloseMultipleVolumes(FormatDBPtr* fdbp_array, 
			       Int4 volume_count)
{
	Int4 volumeI;
	for( volumeI = 0; volumeI < volume_count; volumeI++ ){
		if(FormatDBClose( fdbp_array[ volumeI ] ))
			return 9;
	}
	return 0;
}

/** FD_CreateAliasFileEx2 uses this definition */
#define FILECLOSE(x)  if(x){ FileClose(x); x=NULL;}

/**
 * mpiBLAST support function
 * modifies the original FD_CreateAliasFileEx to use 3 digit fragment identifiers
 */
Boolean FD_CreateAliasFileEx2(CharPtr title, CharPtr basename, 
...
/* mpiBLAST support function
 * This is a hack of FD_ConstructMultivolumeDBList to always use 3 digit
 * fragment identifiers
 */
CharPtr FD_ConstructMultivolumeDBList2(CharPtr basename, Int4 nvols)
{
    CharPtr retval = NULL;
    Int4 i, len = 0;
    Char numstr[10];
...
/* mpiBLAST support function
 * This is a hack of s_SetDBListNameMultiVolDereference to always use 3 digit
 * fragment identifiers
 */
static void
s_SetDBListNameMultiVolDereference2(const ReadDBFILE* rdfp,
                                   CharPtr basename_requested_by_user,
                                   Int4 rdfp_ctr,
                                   Char base_fn[])
{
    ...
    if (options->alias_file_name && options->gi_file) {
        ...
        for (; rdfp; rdfp = rdfp->next, rdfp_ctr++) {
            ...
            if (rdfp->next || rdfp_ctr != 0) {
				/* mpiBLAST specific <<<<< */
                s_SetDBListNameMultiVolDereference2(rdfp, options->base_name,
                                                   rdfp_ctr, base_fn);
				/* mpiBLAST specific >>>>> */
            } else {
			...
			/* mpiBLAST specific <<<<< */
            FD_CreateAliasFileEx2(options->db_title, alias_fn, 0, 
        ...
        if (nletters_tot == 0 || nseqs_tot == 0) {
            ...
            for (i = 0; i <= rdfp_ctr; i++) {
				/* mpiBLAST specific <<<<< */
                sprintf(alias_fn,"%s.%03d.%cal", options->alias_file_name, i,
        ...
        if (rdfp_ctr == 1) { /* single volume database */
			...
			/* mpiBLAST specific <<<<< */
            Char *p = FD_ConstructMultivolumeDBList2(options->alias_file_name,
                    rdfp_ctr);
			/* mpiBLAST specific >>>>> */
             ...
             */
			/* mpiBLAST specific <<<<< */
            FD_CreateAliasFileEx2(options->db_title, options->alias_file_name,
...
//        return 2;        
    /* Initialize multi-volume formatdb structures */
	volume_count = dump_args[numvolumes_arg].intvalue;
	if( volume_count <= 0 ){
		fprintf( stderr, "[formatdb] ERROR: No fragment decomposition specified\n" );
		return 2;
	}
	volume_lengths = (Int8*)MemNew( sizeof(Int8)*volume_count );
	MemSet( volume_lengths, 0, sizeof(Int8)*volume_count );
	fdbp_array = (FormatDBPtr*)MemNew( sizeof(FormatDBPtr) * volume_count );
	if( FDBCreateMultipleVolumes( fdbp_array, options, volume_count ) )
		return 2;
	/* mpiBLAST specific >>>>> */
    ...
    while (options->db_file) {
       ...
       if (!options->isASN) {
          ...
          while ((sep = FastaToSeqEntryForDb(fd, 
		  	 ...
		  	 /* mpiBLAST specific <<<<< */
             int volumeI = 0;
			 Int8 shortest_len = volume_lengths[ 0 ];
			 Int4 shortest_volumeI = 0;
		  	 /* mpiBLAST specific >>>> */
			 ...
			 /* mpiBLAST specific <<<<< */
			 /* add the sequence to the smallest fragment */
			 for( volumeI = 0; volumeI < volume_count; volumeI++ ){
			 	if( volume_lengths[ volumeI ] < shortest_len ){
					shortest_len = volume_lengths[ volumeI ];
					shortest_volumeI = volumeI;
				}
			 }

             bdp = FDBGetDefAsnFromBioseq(bsp, taxid_tbl);
             // if ( FDBAddBioseq(fdbp, bsp, bdp) ) {
             if ( FDBAddBioseq(fdbp_array[shortest_volumeI], bsp, bdp) ) {
			 ...
			 volume_lengths[ shortest_volumeI ] += bsp->length;
             bdp = BlastDefLineSetFree(bdp);
             SeqEntryFree(sep);
			 seq_count++;	/* for debugging */
			 /* mpiBLAST specific >>>> */
          }
      ...
      // FD_MakeAliasFile(options);
		  /* Use FD_CreateAliasFileEx2 instead of FD_MakeAliasFile to get 3 digit
		   * fragment identifiers */
		   FD_CreateAliasFileEx2(options->db_title, options->alias_file_name, options->volume+1, 
    ...
    //    return 9;
	if( FDBCloseMultipleVolumes( fdbp_array, volume_count ) ){
		return 9;
	}
	/* mpiBLAST specific >>>>> */
    ...
    if (multiple_inputs) {
        ...
        for (; input_ctr < num_inputfiles; input_ctr++) {
            while (rdfp) {
                ...
                sprintf(basename, "%s.%03d", inputs[input_ctr], vol_ctr);
				/* mpiBLAST specific >>>>> */
                ...
                ErrLogPrintf("Input %s (up to %ld) alias file %s (%ld-%ld)\n",
                        ...
                        start_oid, stop_oid);
				/* mpiBLAST specific <<<<< */
                FD_CreateAliasFileEx2(NULL, basename, 0, 
            ...
            if (span_multiple_rdfp) {
				...
				/* mpiBLAST specific <<<<< */
                FD_CreateAliasFileEx2(NULL, inputs[input_ctr], 0,
