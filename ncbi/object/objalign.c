...
NLM_EXTERN Boolean LIBCALL SeqAlignSetAsnWrite (SeqAlignPtr sap, AsnIoPtr aip, AsnTypePtr set, AsnTypePtr element)
{
	...
	if(pio_hijacking) {
		AsnIoFlush(aip);
		intercept_output_asn(INT_HDR_END);
	}
    ...
    {
#ifdef MPIBLAST_PIO
		subject_id = SeqIdDup(TxGetSubjectIdFromSeqAlign(sap));
		/* chech whether the start of another sequence */
		if(pio_hijacking && (last_id && SeqIdComp(subject_id, last_id) != SIC_YES)) {
			AsnIoFlush(aip);
			intercept_output_asn(INT_ALN_END);
		}
	...
	if(pio_hijacking) {
		AsnIoFlush(aip);
		intercept_output_asn(INT_FTR_BEGIN);
	}
#endif
