...
#ifdef MPIBLAST_PIO
	if( !options->calculate_statistics_and_exit && parallel_evalue_adjust) {
		search->dblen = global_db_len;
		search->dbseq_num = global_dbseq_num;
	}
#endif
...
#ifdef MPIBLAST_PIO
	if( options->calculate_statistics_and_exit && parallel_evalue_adjust) {
		global_db_len = search->dblen;
		global_dbseq_num = search->dbseq_num;
	}
#endif
...
#ifdef MPIBLAST_PIO
		if( !options->calculate_statistics_and_exit && parallel_evalue_adjust) {
			total_length = global_db_len;
			total_number = global_dbseq_num;
		}
#endif
